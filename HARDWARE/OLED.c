...
static uint8_t s_OledI2cAddr = OLED_I2C_ADDR_WRITE;
...
{
	GPIO_WriteBit(PIN_OLED_SCL_GPIO, PIN_OLED_SCL_PIN, (BitAction)BitValue);
	OLED_I2C_Delay();
}
...
{
	GPIO_WriteBit(PIN_OLED_SDA_GPIO, PIN_OLED_SDA_PIN, (BitAction)BitValue);
	OLED_I2C_Delay();
}
...
void OLED_GPIO_Init(void)
{
	...
	GPIO_InitStructure.GPIO_Pin = PIN_OLED_SCL_PIN | PIN_OLED_SDA_PIN;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_OUT;
	GPIO_InitStructure.GPIO_OType = GPIO_OType_OD;
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_2MHz;
	GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_UP;
	GPIO_Init(PIN_OLED_SCL_GPIO, &GPIO_InitStructure);
...
static void OLED_SDA_SetOutput(void)
{
	GPIO_InitTypeDef gpio;

	GPIO_StructInit(&gpio);
	gpio.GPIO_Pin = PIN_OLED_SDA_PIN;
	gpio.GPIO_Mode = GPIO_Mode_OUT;
	gpio.GPIO_OType = GPIO_OType_OD;
	gpio.GPIO_Speed = GPIO_Speed_2MHz;
	gpio.GPIO_PuPd = GPIO_PuPd_UP;
	GPIO_Init(PIN_OLED_SDA_GPIO, &gpio);
}

static void OLED_SDA_SetInput(void)
{
	GPIO_InitTypeDef gpio;
...
static uint8_t OLED_I2C_ProbeAddr(uint8_t addr)
{
	uint8_t i;
	uint8_t ack;
	...
	OLED_W_SCL(1);
	OLED_I2C_Delay();
	ack = (GPIO_ReadInputDataBit(PIN_OLED_SDA_GPIO, PIN_OLED_SDA_PIN) == Bit_RESET) ? 1U : 0U;
	OLED_W_SCL(0);
	OLED_SDA_SetOutput();
	OLED_W_SDA(1);
...
static void OLED_I2C_SelectAddr(void)
{
	if (OLED_I2C_ProbeAddr(OLED_I2C_ADDR_WRITE) != 0U)
	{
		s_OledI2cAddr = OLED_I2C_ADDR_WRITE;
		return;
	}
	if (OLED_I2C_ProbeAddr(OLED_I2C_ADDR_ALT) != 0U)
	{
		s_OledI2cAddr = OLED_I2C_ADDR_ALT;
		return;
	}
...
void OLED_WriteCommand(uint8_t Command)
{
	...
	OLED_I2C_SendByte(s_OledI2cAddr);	//OLED I2C从机地址（8-bit写）
...
void OLED_Init(void)
{
...
#if (OLED_CTRL_SSD1309 != 0U)
	/* SSD1309：必须先解锁命令接口；勿发 SSD1306 电荷泵命令 0x8D */
	...
	OLED_WriteCommand(0x40);	//起始行0

#if (OLED_CTRL_SSD1309 != 0U)
	OLED_WriteCommand(0x20);	//寻址模式
	OLED_WriteCommand(0x00);	//水平寻址
#else
	OLED_WriteCommand(0x8D);	//电荷泵（SSD1306 内部供电）
	OLED_WriteCommand(0x14);
	OLED_WriteCommand(0x20);	//寻址模式
	OLED_WriteCommand(0x02);	//页寻址（与 OLED_SetCursor 一致）
#endif
	...
	OLED_WriteCommand(0xA4);	//全屏点亮跟随RAM
...
void OLED_ShowFloatNum(uint8_t X, uint8_t Y, double Number, uint8_t IntLength, uint8_t FraLength, uint8_t FontSize)
{
	...
	FraNum = (uint32_t)(Number * (double)PowNum + 0.5);	//四舍五入，避免显示误差
